  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      for (var i = 1; i <= reqCount; i++) {
        ...
        // req is [sellerId, buyerId, landId, requestStatus]
        var sellerId = req[0];
        var buyerId = req[1];
        var landId = req[2];
        var isApproved = req[3];

        if (buyerId.toLowerCase() === currentAddress.toLowerCase() && isApproved) {
          var paid = await this.state.LandInstance.methods.isPaid(landId).call();
          var price = await this.state.LandInstance.methods.getPrice(landId).call();
          localRow.push(
            <tr key={"req-" + i}>
              <td>{landId}</td>
              <td>{sellerId}</td>
              <td>{price}</td>
              <td>
                <Button onClick={this.makePayment(sellerId, price, landId)}
