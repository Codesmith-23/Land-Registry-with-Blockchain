...
contract Land {
    ...
    mapping(uint => address) public LandHeir;
    mapping(uint => bool) public OwnerDeceased;
    ...
    event HeirDesignated(uint _landId, address _heir);
    event OwnerMarkedDeceased(uint _landId);
    event InheritanceClaimed(uint _landId, address _newOwner);
    ...
    function LandOwnershipTransfer(uint _landId, address _newOwner) public{
        require(isLandInspector(msg.sender));
        require(PaymentReceived[_landId] == true, "Transfer failed: Buyer payment is pending");
    ...
    // ===== Land Inheritance Feature =====

    function designateHeir(uint _landId, address _heir) public {
        require(LandOwner[_landId] == msg.sender, "Only the land owner can designate an heir");
        require(_heir != address(0), "Heir address cannot be zero");
        LandHeir[_landId] = _heir;
        emit HeirDesignated(_landId, _heir);
    }

    function markDeceased(uint _landId) public {
        require(isLandInspector(msg.sender), "Only Land Inspector can mark as deceased");
        OwnerDeceased[_landId] = true;
        emit OwnerMarkedDeceased(_landId);
    }

    function claimInheritance(uint _landId) public {
        require(LandHeir[_landId] == msg.sender, "Only the designated heir can claim");
        require(OwnerDeceased[_landId] == true, "Owner is not marked as deceased");
        LandOwner[_landId] = msg.sender;
        LandHeir[_landId] = address(0);
        OwnerDeceased[_landId] = false;
        emit InheritanceClaimed(_landId, msg.sender);
    }

    function getHeir(uint _landId) public view returns (address) {
        return LandHeir[_landId];
    }

    function isDeceased(uint _landId) public view returns (bool) {
        return OwnerDeceased[_landId];
    }
